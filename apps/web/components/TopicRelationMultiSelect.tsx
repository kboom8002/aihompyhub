...
export function TopicRelationMultiSelect({ value = [], onChange }: { value: string[], onChange: (val: string[]) => void }) {
  ...
  useEffect(() => {
    fetch('/api/v1/tenant/content?category=brand_ssot&type=topic_hub', { headers: { 'x-tenant-id': '00000000-0000-0000-0000-000000000001' } })
      .then(res => res.json())
  ...
  const filtered = topics.filter(t => !value.includes(t.id) && t.title.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
      <div ref={wrapperRef} style={{ position: 'relative', width: '100%', border: '1px solid #cbd5e1', borderRadius: '4px', padding: '0.75rem', background: 'white' }}>
                ...
                {value.map(id => {
                    ...
                    return (
                        <div key={id} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#e2e8f0', padding: '6px 12px', borderRadius: '16px', fontSize: '0.85rem', fontWeight: 600, color: '#334155' }}>
                           <span style={{ marginRight: '2px' }}>💡</span> <span>{t ? t.title : '불러오는 중...'}</span>
          ...
          <input 
             ...
             placeholder="🔍 연결할 지식의 키워드를 쳐서 검색하세요 (예: 여드름)"
