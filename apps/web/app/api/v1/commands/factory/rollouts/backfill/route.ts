...
const BackfillRolloutSchema = z.object({
  meta: z.object({
    requestId: z.string(),
...
export async function POST(request: Request) {
  try {
    ...
    const { meta: commandMeta, body: payload } = BackfillRolloutSchema.parse(rawBody);
    ...
    const { error } = await supabase
      ...
      .update({ status: 'rolling_out' }) // Re-activate back to rolling_out queue
      .eq('id', payload.rolloutId);

    if (error) throw new Error(`DB Error: ${error.message}`);

    console.log(`[Factory Control] Backfill DB applied for Rollout ${payload.rolloutId}.`);
