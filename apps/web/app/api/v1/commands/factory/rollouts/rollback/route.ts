...
const RollbackRolloutSchema = z.object({
  meta: z.object({
    requestId: z.string(),
...
export async function POST(request: Request) {
  try {
    ...
    const { meta: commandMeta, body: payload } = RollbackRolloutSchema.parse(rawBody);
    ...
    console.log(`[Factory Control] DB updated: Rolling back Rollout ${payload.rolloutId}. Reason: ${payload.reason}`);
