...
export default function UniversalContentListView() {
  ...
  const loadContents = () => {
    fetch(`/api/v1/tenant/content?category=${category}&type=${type}`, { headers: { 'x-tenant-id': '00000000-0000-0000-0000-000000000001' } })
      .then(res => res.json())
      .then(payload => {
        if (payload?.data) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/tenant/content?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
         loadContents();
      }
    } catch {
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        />
        <Link 
           href={`/tenant/studio/${category}/${type}/new`}
                ...
                <tr key={c.id}>
                  ...
                  <td style={{ padding: '1.25rem 0', borderBottom: '1px solid #e5e7eb', textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      <Link href={`/tenant/studio/${category}/${type}/${c.id}/edit`} title="수정" style={{ padding: '0.35rem', background: '#e0f2fe', color: '#0284c7', border: 'none', borderRadius: '4px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', textDecoration: 'none' }}>
