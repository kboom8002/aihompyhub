...
export default async function RoutinesIndexPage(props: { params: Promise<{ tenantSlug: string }> }) {
  ...
  return (
    <div className="container mx-auto py-16 px-4 max-w-4xl">
      <Link href={`/${params.tenantSlug}`} className="text-sm border border-[var(--theme-border)] px-4 py-2 rounded-full mb-12 inline-block hover:bg-black/5 transition-colors font-medium">
        &larr; 홈으로 돌아가기
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {(!assets || assets.length === 0) && (
           <div className="col-span-full py-12 text-center border rounded-lg bg-[var(--theme-surface)]">
             <p className="opacity-60">아직 작성된 루틴이 없습니다.</p>
           </div>
        )}
        
        {assets?.map((routine) => (
          <Link href={`/${params.tenantSlug}/routines/${routine.id}`} key={routine.id}>
