...
export default async function CompareIndexPage(props: { params: Promise<{ tenantSlug: string, locale?: string }> }) {
  ...
  return (
    <div className="container mx-auto py-16 px-4 max-w-5xl">
      ...
      <div className="mb-12 border-b border-[var(--theme-border)] pb-8">
        <h1 className="text-4xl font-light tracking-tight font-[family-name:var(--theme-font)] mb-4">비교(VS) 허브</h1>
        <p className="opacity-70">당신에게 더 잘 맞는 제품/루틴을 객관적으로 비교합니다.</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {assets?.map((asset: any) => (
          <Link href={`/${locale}/${params.tenantSlug}/compare/${asset.id}`} key={asset.id}>
             <div className="group h-full bg-[var(--theme-surface)] p-6 rounded-[var(--theme-radius)] border border-[var(--theme-border)] shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer flex flex-col gap-2">
                 ...
                 <div className="text-xs font-bold text-[var(--theme-primary)] tracking-widest uppercase mb-1">
                    {asset.json_payload?.type || 'COMPARE SSoT'}
                 </div>
